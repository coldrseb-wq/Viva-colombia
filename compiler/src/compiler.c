...
static Compiler* init_compiler(const char* outfile, OutMode mode) {
    ...
    if (mode == OUT_C) {
        fprintf(c->f,
            ...
            "// Bootstrap helper: itoa\n"
            "char* viva_itoa(int n){static char buf[32];snprintf(buf,32,\"%%d\",n);return buf;}\n"
            "void prtchr(char c){printf(\"%%c\",c);}\n\n");
...
static void compile_expr(Compiler* c, ASTNode* n) {
    ...
    // File I/O as expressions (Phase 6)
    else if (n->type == FILE_OPEN_NODE) {
        if (c->mode == OUT_C) {
            emit(c, "fopen(");
            compile_expr(c, n->left);
            emit(c, ", ");
    ...
    // Standard library as expressions
    else if (n->type == STRLEN_NODE) {
        if (c->mode == OUT_C) {
            emit(c, "strlen(");
            compile_expr(c, n->left);
            emit(c, ")");
    ...
    // Bootstrap support expressions
    else if (n->type == ORD_NODE) {
        if (c->mode == OUT_C) {
            emit(c, "(int)(");
            compile_expr(c, n->left);
            emit(c, ")");
    ...
    else if (n->type == ITOA_NODE) {
        if (c->mode == OUT_C) {
            // itoa returns a static buffer - use viva_itoa helper
            emit(c, "viva_itoa(");
            compile_expr(c, n->left);
            emit(c, ")");
        }
    }
    else if (n->type == WRITE_BYTE_NODE) {
        if (c->mode == OUT_C) {
            emit(c, "fputc(");
            compile_expr(c, n->right);  // byte
            emit(c, ", ");
            compile_expr(c, n->left);   // file
            emit(c, ")");
        }
    }
}
...
static void compile_var(Compiler* c, ASTNode* n, int spanish) {
    ...
    if (c->mode == OUT_C) {
        ...
        if (n->left && (n->left->type == STRING_LITERAL_NODE ||
            ...
            compile_expr(c, n->left);
            emit(c, ";\n");
        } else if (n->left && n->left->type == ITOA_NODE) {
            // itoa returns char*
            emit(c, "char* %s = ", n->value);
            compile_expr(c, n->left);
            emit(c, ";\n");
            is_str = 1;
        } else if (n->left && n->left->type == CHR_NODE) {
            // chr returns char
            emit(c, "char %s = ", n->value);
            compile_expr(c, n->left);
            emit(c, ";\n");
    ...
    while (n) {
        switch (n->type) {
            ...
            case CONTINUE_NODE:
                ...
                break;
            // File I/O operations (Phase 6)
            case FILE_DECL_NODE:
                if (c->mode == OUT_C) {
                    indent(c);
                    emit(c, "FILE* %s", n->value);
                    if (n->left) {
                        emit(c, " = ");
                        if (n->left->type == FILE_OPEN_NODE) {
                            emit(c, "fopen(");
                            compile_expr(c, n->left->left);
                            emit(c, ", ");
